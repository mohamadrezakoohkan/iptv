...
description: Phase 2 (IMPLEMENT) of the CORE_FLOW orchestration harness. Implements one task — production code plus unit tests (the only test tier). Spawn ONLY from the orchestrator pipeline defined in CORE_FLOW.md, one task per spawn.
...
4. **Write the unit tests the task demands** — unit tests are the only test
   tier in this harness. Tests assert the acceptance criteria, not
   implementation details. Run them yourself while you work — handing knowingly
   red tests to validation burns the retry budget.
