...
attribute on the document root; see `docs/specs/theme.md` and ADR-0019. Every rule
...
The design ships `src/client/api.js` as a self-contained IIFE that exposes
`window.IptvApi`. The server-side proxy in `src/server/rtr.js` forwards
...
1. When a channel card is clicked, `src/client/play.js` is called with the
...
2. **Engine selection by stream type** — decided from the URL path
   ...
   - anything else (notably `.ts`, the Xtream live format) → MPEG-TS
     engine (mpegts.js 1.7+ via CDN, loaded in `src/index.html` alongside
     hls.js).
...
`src/server/hls.js` (mounted from `src/server/rtr.js`) converts a raw MPEG-TS
...
Phases are defined in `src/client/st.js` per CONVENTIONS.md §6:
