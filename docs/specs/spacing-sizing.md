...
only files that change are `src/client/app.css` (the rules) and — only if any
hardcoded size lives in markup — `src/index.html`; no other client file is touched.

## 1. Token contract

Eight new custom properties are declared once on `:root` in `src/client/app.css`,
