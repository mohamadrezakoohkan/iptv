...
- The dark theme reuses the existing `:root` design-token values verbatim
  (see `docs/specs/iptv-player.md` §2). The light theme re-defines the same eight
...
- The light token values live in a CSS rule selected by
  `:root[data-theme="light"]` in `src/client/app.css`, overriding the dark
...
- `src/index.html` ships with no `data-theme` on `<html>` (baseline = dark — see
...
- The chosen theme persists in `localStorage` under the key `iptv_theme`
  (declared as `S.themeKey` in `src/client/cfg.js`, matching the `iptv_*` /
...
- The read/write helpers live in `src/client/st.js` alongside the other persistence
...
- On page load (`src/client/main.js`, `onReady`), the stored theme is read and
