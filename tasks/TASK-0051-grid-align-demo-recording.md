...
_Filled by implement-agent. Note: `playwright.config.js` already sets
`outputDir: '.playwright-out'` (gitignored), so committed artifacts under
`test-results/` survive the run (per the E13 remediation lesson)._
